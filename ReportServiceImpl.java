...
public class ReportServiceImpl implements ReportService {
	...
	public List<Map<String, Object>> getSalesReport(String type, String value) {
	    ...
	    if (type.equalsIgnoreCase("Daily")) {
	        invoices = invoiceRepository.findByDate(LocalDate.parse(value));
	    } else if (type.equalsIgnoreCase("Monthly")) {
	    	YearMonth month = YearMonth.from(LocalDate.parse(value)); // ✅ safe conversion
// "2025-07"
	        LocalDate start = month.atDay(1);
	        LocalDate end = month.atEndOfMonth();
	        invoices = invoiceRepository.findByDateBetween(start, end);
	    } else {
	        return data;
	    }
	...
	public List<Map<String, Object>> getPurchaseReport(String type, String value) {
	    ...
	    if (type.equalsIgnoreCase("Daily")) {
	        purchases = purchaseRepository.findByPurchaseDate(LocalDate.parse(value));
	    } else if (type.equalsIgnoreCase("Monthly")) {
	    	YearMonth month = YearMonth.from(LocalDate.parse(value)); // ✅ safe conversion
 // "2025-07"
	        LocalDate start = month.atDay(1);
	        LocalDate end = month.atEndOfMonth();
	        purchases = purchaseRepository.findByPurchaseDateBetween(start, end);
	    } else {
	        return data;
	    }
	...
	public List<Map<String, Object>> getProductWiseSales(String productName) {
	    List<InvoiceItem> items = invoiceItemRepository.findByProduct_Name(productName);
	    ...
	    for (InvoiceItem item : items) {
	        ...
	        // ✅ Safely handle null invoice
	        if (item.getInvoice() != null && item.getInvoice().getDate() != null) {
	            row.put("date", item.getInvoice().getDate().toString());
	        } else {
	            row.put("date", "N/A");
	        }
	...
	public List<Map<String, Object>> getCustomerHistory(String customerName) {
	    List<Invoice> invoices = invoiceRepository.findByCustomerName(customerName);
    ...
    public void downloadPdfReport(String type, String value, HttpServletResponse response) {
        try {
            List<Map<String, Object>> reportData = getSalesReport(type, value); // Modify as needed
