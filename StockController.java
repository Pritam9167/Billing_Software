...
public class StockController {
    ...
    public String showLowStock(Model model) {
        ...
        List<Product> lowStockProducts = productRepository.findByQuantityInStockLessThanEqual(threshold);
        model.addAttribute("lowStockProducts", lowStockProducts);
        model.addAttribute("threshold", threshold);
        return "low_stock"; // This must match the filename without .html
    }


    @GetMapping("/low/pdf")
    public void exportLowStockToPdf(HttpServletResponse response) throws Exception {
        List<Product> lowStockProducts = productRepository.findByQuantityInStockLessThanEqual(5);
